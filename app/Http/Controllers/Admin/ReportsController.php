...
class ReportsController extends Controller
{
    ...
    {
        // Redirect to the new enhanced inquiry management page
    ...
    private function getActivityTrends()
    {
        ...
        // Get monthly broker registrations
        $brokerRegistrations = User::where('role', 'broker')
        ...
        $propertyListings = Property::whereBetween('created_at', [$startDate, $endDate])
            ->selectRaw('DATE_FORMAT(created_at, "%Y-%m") as month, COUNT(*) as count')
            ->groupBy('month')
            ->orderBy('month')
            ->pluck('count', 'month')
        ...
        // Generate array of months for consistent data structure
        $months = [];
        ...
        foreach ($months as $month) {
            $brokerData[] = $brokerRegistrations[$month] ?? 0;
            $propertyData[] = $propertyListings[$month] ?? 0;
            $inquiryData[] = $inquiries[$month] ?? 0;
            $transactionData[] = $transactions[$month] ?? 0;
        }
    ...
    private function getPropertyStats()
    {
        return [
            'total_properties' => Property::count(),
            'active_properties' => Property::where('status', 'active')->count(),
            'avg_price' => Property::where('status', 'active')->avg('total_price') ?? 0,
            'total_views' => 15420,
        ];
    }
    ...
    private function getRecentPropertyActivities()
    {
        return [
            [
                'type' => 'property_listing',
                'description' => 'New property listed: Residential Lot in Tagbilaran',
                'timestamp' => now()->subHours(2),
            ],
            [
                'type' => 'property_update',
                'description' => 'Property updated: Beachfront in Panglao',
