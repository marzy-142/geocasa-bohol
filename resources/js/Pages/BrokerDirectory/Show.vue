            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col md:flex-row items-center gap-8">
                    ...
                    <div class="flex-1 text-center md:text-left">
                        ...
                        <p v-if="broker.brokerage_firm_name" class="text-xl text-blue-100 mb-4">
                            {{ broker.brokerage_firm_name }}
                        </p>
                        
                        <div class="flex flex-wrap gap-4 justify-center md:justify-start mb-4">
                            <div v-if="broker.city" class="flex items-center text-blue-100">
                                <MapPinIcon class="w-5 h-5 mr-2" />
                                <span>{{ broker.city }}, {{ broker.province }}</span>
                            </div>
                            <div v-if="broker.years_experience" class="flex items-center text-blue-100">
                                <BriefcaseIcon class="w-5 h-5 mr-2" />
                                <span>{{ broker.years_experience }} years experience</span>
                            </div>
                        </div>

                        <div :class="['inline-flex items-center px-4 py-2 rounded-full text-sm font-medium', getAvailabilityColor(broker.availability_status)]">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg p-8">
                        ...
                        <p v-if="broker.bio" class="text-gray-700 leading-relaxed whitespace-pre-wrap">
                            {{ broker.bio }}
                        </p>
                    ...
                    <!-- Specializations -->
                    <div v-if="broker.specializations && broker.specializations.length > 0" class="bg-white rounded-2xl shadow-lg p-8">
                    ...
                    <!-- Service Areas -->
                    <div v-if="broker.service_areas && broker.service_areas.length > 0" class="bg-white rounded-2xl shadow-lg p-8">
                    ...
                    <!-- Active Listings -->
                    <div v-if="broker.properties && broker.properties.length > 0" class="bg-white rounded-2xl shadow-lg p-8">
                        <h2 class="text-2xl font-bold text-gray-900 mb-6">Active Listings</h2>
                        ...
                        <div v-if="broker.active_listings > broker.properties.length" class="mt-6 text-center">
                            <p class="text-gray-600">
                                + {{ broker.active_listings - broker.properties.length }} more listings
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg p-6">
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-2xl font-bold text-blue-600">{{ broker.active_listings }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-gray-600">Total Listings</span>
                                <span class="text-2xl font-bold text-gray-900">{{ broker.total_listings }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-gray-600">Properties Sold</span>
                                <span class="text-2xl font-bold text-green-600">{{ broker.sold_properties }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg p-6">
                        ...
                        <div class="space-y-4">
                            <div v-if="broker.prc_license_number" class="flex items-start">
                                <CheckBadgeIcon class="w-5 h-5 text-green-600 mr-3 mt-0.5" />
                                <div>
                                    <p class="text-sm text-gray-600">PRC License</p>
                                    <p class="font-medium text-gray-900">{{ broker.prc_license_number }}</p>
                                </div>
                            </div>

                            <div v-if="broker.show_phone && broker.office_contact_number" class="flex items-start">
                                <PhoneIcon class="w-5 h-5 text-blue-600 mr-3 mt-0.5" />
                                <div>
                                    <p class="text-sm text-gray-600">Phone</p>
                                    <a :href="`tel:${broker.office_contact_number}`" class="font-medium text-blue-600 hover:text-blue-800">
                                        {{ broker.office_contact_number }}
                                    </a>
                                </div>
                            </div>

                            <div v-if="broker.show_email && broker.email" class="flex items-start">
                                <EnvelopeIcon class="w-5 h-5 text-blue-600 mr-3 mt-0.5" />
                                <div>
                                    <p class="text-sm text-gray-600">Email</p>
                                    <a :href="`mailto:${broker.email}`" class="font-medium text-blue-600 hover:text-blue-800 break-all">
                                        {{ broker.email }}
                                    </a>
                                </div>
                            </div>
                            ...
                            <div v-if="broker.website" class="flex items-start">
                                <GlobeAltIcon class="w-5 h-5 text-blue-600 mr-3 mt-0.5" />
                                <div>
                                    <p class="text-sm text-gray-600">Website</p>
                                    <a :href="broker.website" target="_blank" class="font-medium text-blue-600 hover:text-blue-800 break-all">
                                        {{ broker.website }}
                                    </a>
                                </div>
                            </div>
                        ...
                        <!-- Social Links -->
                        <div v-if="broker.facebook || broker.linkedin" class="mt-6 pt-6 border-t border-gray-200">
                            <p class="text-sm text-gray-600 mb-3">Connect on Social Media</p>
                    ...
                    <!-- Contact CTA -->
                    <div v-if="broker.accept_inquiries" class="bg-gradient-to-br from-blue-600 to-purple-600 rounded-2xl shadow-lg p-6 text-white">
                        <h3 class="text-lg font-bold mb-2">Interested in working together?</h3>
                        <p class="text-blue-100 mb-4">Send {{ broker.name.split(' ')[0] }} a message and start your property journey today.</p>
                        <button
                            @click="showContactModal = true"
            ...
            <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-2xl font-bold text-gray-900">Contact {{ broker.name }}</h3>
                        <button
                    ...
                    <form @submit.prevent="submitInquiry" class="space-y-4">
                        ...
                        <div>
                            ...
                            <p v-if="form.errors.inquiry_type" class="mt-1 text-sm text-red-600">{{ form.errors.inquiry_type }}</p>
                        </div>
